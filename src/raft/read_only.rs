...
use crate::raft::raftpb::raft::{MessageType, Message};
use std::collections::HashMap;
use crate::raft::raft::ReadOnlyOption;
use std::borrow::Cow;
...
impl ReadOnly {
    ...
    pub(crate) fn add_request(&mut self, index: u64, m: Message) {
        let s = m.get_entries()[0].get_Data();
        let read_index_status = ReadIndexStatus {
            req: m.clone(),
            index,
            acks: Default::default(),
        };
        self.pending_read_index.entry(s.to_vec()).or_insert(read_index_status);
        self.read_index_queue.push(s.to_vec());
    }
    ...
    fn last_pending_request(&self) -> String {
        if self.read_index_queue.is_empty() { return "".to_string(); }
        String::from_utf8_lossy(self.read_index_queue.last().unwrap()).to_string()
    }
}